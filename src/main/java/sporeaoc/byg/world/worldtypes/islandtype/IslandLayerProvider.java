...
import sporeaoc.byg.world.worldtypes.islandtype.layers.IslandSizer;
import sporeaoc.byg.world.worldtypes.islandtype.layers.MasterOceanLayer;
import sporeaoc.byg.world.worldtypes.islandtype.layers.OceanEdges;
...
public class IslandLayerProvider {
    private static <A extends IArea, R extends IExtendedNoiseRandom<A>> IAreaFactory<A> stackLayers(LongFunction<R> randomProvider) {
        ...
        //islandFactory = ZoomLayer.NORMAL.apply(randomProvider.apply(1000L), islandFactory);
        islandFactory = OceanEdges.INSTANCE.apply(randomProvider.apply(72421L), islandFactory);
        for (int oceanBiomeSize = 0; oceanBiomeSize <= 5; oceanBiomeSize++) {
        ...
        if (!BYGIslandSettingsConfig.survivalIslandMode.get()) {
        ...
        islandFactory = ZoomLayer.NORMAL.apply(randomProvider.apply(30950L), islandFactory);
        islandFactory = IslandSizer.INSTANCE.apply(randomProvider.apply(9123L), islandFactory);
        for (int islandSize = 0; islandSize <= 8; islandSize++) {
            islandFactory = IslandSizer.INSTANCE.apply(randomProvider.apply(9123L), islandFactory);
        }
