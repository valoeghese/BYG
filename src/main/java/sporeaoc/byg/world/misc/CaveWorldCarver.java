package sporeaoc.byg.world.misc;
...
public class CaveWorldCarver extends WorldCarver<ProbabilityConfig> {
    public CaveWorldCarver(Function<Dynamic<?>, ? extends ProbabilityConfig> config, int p_i49929_2_) {
        super(config, p_i49929_2_);
    }
    ...
    public boolean func_225555_a_(IChunk chunk, Function<BlockPos, Biome> biomefunctionThing, Random rand, int sealevel, int cX, int cZ, int chunkX, int chunkZ, BitSet carvingMask, ProbabilityConfig config) {
        int i = (this.func_222704_c() * 2 - 1) * 16;
        int j = rand.nextInt(rand.nextInt(rand.nextInt(this.func_222724_a()) + 1) + 1);

        for (int k = 0; k < j; ++k) {
            double d0 = (double) (cX * 16 + rand.nextInt(16));
            double d1 = (double) this.generateCaveStartY(rand);
            double d2 = (double) (cZ * 16 + rand.nextInt(16));
            int l = 1;
            if (rand.nextInt(4) == 0) {
                double d3 = 0.5D;
                float f1 = 1.0F + rand.nextFloat() * 6.0F;
                this.func_227205_a_(chunk, biomefunctionThing, rand.nextLong(), sealevel, chunkX, chunkZ, d0, d1, d2, f1, 0.5D, carvingMask);
                l += rand.nextInt(4);
            }
            ...
            for (int k1 = 0; k1 < l; ++k1) {
                ...
                int j1 = 0;
                this.func_227206_a_(chunk, biomefunctionThing, rand.nextLong(), sealevel, chunkX, chunkZ, d0, d1, d2, f2, f, f3, 0, i1, this.func_222725_b(), carvingMask);
            }
        }
