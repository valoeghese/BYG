...
public class CypressTree extends AbstractTreeFeature<NoFeatureConfig> {

    private final int minHeight;
    private final BlockState log;
    private final BlockState leaves;

    public CypressTree(final Function<Dynamic<?>, ? extends NoFeatureConfig> function, final boolean worldGen) {
        this(function, worldGen, 15);
    }

    public CypressTree(final Function<Dynamic<?>, ? extends NoFeatureConfig> function, final boolean worldGen, int minHeight) {
        this(function, worldGen, minHeight, BlockCatalogs.CYPRESS_LOG.getDefaultState(), BlockCatalogs.CYPRESS_LEAVES.getDefaultState());
    }

    public CypressTree(final Function<Dynamic<?>, ? extends NoFeatureConfig> function, final boolean worldGen, int minHeight, BlockState log, BlockState leaves) {
    ...
    protected boolean place(Set<BlockPos> blocks, IWorldGenerationReader world, Random random, BlockPos pos, MutableBoundingBox box) {
        int height = random.nextInt(50) + minHeight;
